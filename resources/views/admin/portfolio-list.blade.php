    ...
    <div class="container-fluid">
      <div class="card">
        <div class="card-header">
          <a href="{{ url(ADMINPATH.'add-portfolio') }}" class="btn btn-success m-auto float-right">Add Portfolio</a>
        </div>
        <div class="card-body">
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '{{url(ADMINPATH.'portfolio-data')}}?' + qparam,
            type: "POST",
            headers: {
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '{{url(ADMINPATH.'portfolio-data')}}' + newQueryParam,
                headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
             },
                "type": 'POST',
            ...
            { data: "project_category", "title": "Category" },
            { data: "id", "title": "Project Details","render":project_details },
    ...
    var project_details = ( data, type, row, meta )=>{
  var data = '';
  let project_name= row.project_name!=null?row.project_name:"";
  let project_url= row.project_url!=null?row.project_url:"";
  if(type === 'display'){
        data += '<span class="fotr_10"><b>Added On : </b>'+project_name+'</span><br>' ;
        data += '<span class="fotr_10"><b>Updated On : </b><a href="'+project_url+'" target="_anoop">'+project_url+'</a></span>' ;
...
function action_render(data, type, row, meta) {
    let output = '';
    if (type === 'display') {
        output = '<a href="{{url(ADMINPATH."add-portfolio?id=")}}' + row.id + '" class="btn btn-success btn-sm text-white " title="Edit State"><i class="fa fa-edit"></i></a> ';
    }
    return output;
}
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'tbl_portfolio_list')`;
