    ...
    <div class="container-fluid">
      <div class="card">
        <div class="card-header">
          <a href="{{ url(ADMINPATH.'add-experience') }}" class="btn btn-success m-auto float-right">Add Experience</a>
        </div>
        <div class="card-body">
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '{{url(ADMINPATH.'experience-data')}}?' + qparam,
            type: "POST",
            headers: {
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '{{url(ADMINPATH.'experience-data')}}' + newQueryParam,
                headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
             },
                "type": 'POST',
            ...
            { data: "from_year", "title": "From Year" },
            { data: "to_year", "title": "To Year" },
...
function action_render(data, type, row, meta) {
    let output = '';
    if (type === 'display') {
        output = '<a href="{{url(ADMINPATH."add-experience?id=")}}' + row.id + '" class="btn btn-success btn-sm text-white " title="Edit State"><i class="fa fa-edit"></i></a> ';
    }
    return output;
}
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'tbl_experience_list')`;
